...
protocol OptionsUIDelegate {
    func uiDidSelect(object: Options)
}

class OptionsUI : UIView {
    var delegate: OptionsUIDelegate!
    
    var parentData : OptionsData? = nil
    var objects : [Options]?
    var filtered : [Options]?
    var cellIdentifier = "OptionsCellId"
...
extension OptionsUI: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let selectedOption = PersistenceManager.getOption(key: parentData!.slug)
        cell.textLabel?.text = category.name
        cell.accessoryType = (selectedOption != nil && selectedOption?.slug == category.slug) ? .checkmark : .none
        return cell
    }
}

extension OptionsUI: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        tableView.deselectRow(at: indexPath, animated: true);
        if filtered != nil {
            delegate?.uiDidSelect(object: self.filtered![indexPath.row])
        } else {
            delegate?.uiDidSelect(object: self.objects![indexPath.row])
        }
    }
}

extension OptionsUI: UISearchBarDelegate {
